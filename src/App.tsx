...
export const INTRIGUES: Intrigue[] = [
  ...
  { id: "forbidden-devotee", name: "Devoção Elemental de Luna, Toryama, Haika Kimira e Anjo Caído", icon: "◆", description: "Termine com pelo menos 1 Item Proibido no Museu.", reward: 5, target: 1, metric: "legendaryRelics" },
  ...
  { id: "obsessive-collector", name: "Coleção de Olivia, Laya, Veronica e Ana Clara", icon: "▤", description: "Termine com pelo menos 5 relíquias no Museu.", reward: 5, target: 5, metric: "collection" },
...
export const LEGENDARY_RELICS: Relic[] = [
  { id: "dragon-heart", name: "Coração do Dragão de Toryama", epithet: "Uma criatura de tinta que aprendeu a pulsar", icon: "◆", prestige: 6, start: 5, tags: ["Guerra", "Riqueza"], legendary: true, lore: "Toryama desenhou o coração; Daniel Ramos ensinou o dragão a sobreviver.", power: { name: "Dízimo do Dragão de Toryama", description: "Roube 2 moedas de cada rival.", type: "tax", once: "game", value: 2 } },
...
export const FUSION_RECIPES: FusionRecipe[] = [
  ...
  { id: "recipe-profane-communion", components: ["last-goblet", "heretic-grimoire"], tier: 2, cost: 2, result: makeFusion("fusion-profane-communion", "Comunhão Dissonante de Ximbinha e Toryama", "♨", 6, ["Fé", "Morte", "Oculto"], "Absolvição pela Tinta", "Pague 3 moedas, ganhe 3 Prestígios e purifique uma maldição.", "game", "ownRelic") },
  ...
  { id: "recipe-draconic-banner", components: ["dragon-heart", "hollow-legion-banner"], tier: 2, cost: 2, result: makeFusion("fusion-draconic-banner", "Estandarte Dracônico de Toryama e Cajango", "⚑", 10, ["Guerra", "Riqueza", "Morte"], "Marcha do Dragão", "Cada rival paga 2 moedas; moedas não pagas viram Prestígio.", "game", undefined, { legendary: true, curse: { name: "Terra Queimada", description: "Sua próxima renda é reduzida em 3 moedas.", incomePenalty: 3 } }) },
...
function RelicLibraryCard({ relic, forbidden = false }: { relic: Relic; forbidden?: boolean }) {
  ...
  return <article className={`library-card relic-library-card ${relic.cursed ? "cursed" : ""} ${forbidden ? "forbidden" : ""} ${relic.id === "fabiana-creator" ? "creator" : ""}`}>
    <header><span className="library-icon">{relic.icon}</span><div><small>{forbidden ? "ITEM PROIBIDO · candidato à votação" : relic.epithet}</small><h2>{relic.name}</h2><p>{forbidden ? relic.epithet : `“${relic.lore}”`}</p></div><b>✦ {relic.prestige}</b></header>
