...
const RELICS: Relic[] = [
  ...
  { id: "crimson-perfume", name: "Perfume da Rosa Carmesim", epithet: "Vontade transformada em obsessão", icon: "❦", prestige: 2, start: 1, tags: ["Desejo", "Oculto"], lore: "Uma gota convence. Duas gotas condenam.", power: { name: "Encanto Irresistível", description: "Um rival fica fora do próximo leilão.", type: "silence", once: "game", target: true } },
  { id: "widow-ring", name: "Anel da Viúva Rubra", epithet: "A joia de sete funerais", icon: "◉", prestige: 3, start: 2, tags: ["Morte", "Traição"], lore: "Todos os donos morreram procurando por ele.", power: { name: "Cobrar Herança", description: "Receba 2 moedas do espólio da viúva.", type: "gainGold", once: "act", value: 2 } },
  { id: "last-goblet", name: "Cálice do Último Banquete", epithet: "Ainda morno desde a queda da Cidade Santa", icon: "♨", prestige: 2, start: 2, tags: ["Fé", "Morte"], cursed: true, curse: { name: "Sede dos Mortos", description: "Sua renda é reduzida em 1 moeda a cada ato.", incomePenalty: 1 }, lore: "O vinho secou há séculos. A mancha, não.", power: { name: "Último Brinde", description: "Pague 3 moedas e receba 3 Prestígio.", type: "convert", once: "game", value: 3 } },
  { id: "heretic-grimoire", name: "Grimório da Santa Herege", epithet: "Orações escritas de trás para frente", icon: "▤", prestige: 3, start: 3, tags: ["Fé", "Oculto"], lore: "Cada página absolve um pecado ainda não cometido.", power: { name: "Absolvição Profana", description: "Anule a maldição da próxima relíquia amaldiçoada.", type: "ward", once: "game" } },
  { id: "faceless-mirror", name: "Espelho do Príncipe Sem Rosto", epithet: "Ele reflete aquilo que você esconde", icon: "◇", prestige: 2, start: 2, tags: ["Oculto", "Desejo"], cursed: true, curse: { name: "Reflexo Deformado", description: "Enquanto ativa, esta relíquia sofre −1 Prestígio.", penalty: 1 }, lore: "Seu reflexo sempre chega um instante atrasado.", power: { name: "Reflexo da Corte", description: "Ganhe 2 Prestígio se não estiver liderando; caso contrário, ganhe 1.", type: "reflect", once: "act" } },
  { id: "ivory-mask", name: "Máscara de Marfim", epithet: "O rosto de quem nunca existiu", icon: "☽", prestige: 2, start: 1, tags: ["Oculto", "Realeza"], lore: "No interior há marcas de unhas apontando para fora.", power: { name: "Identidade Falsa", description: "Seu próximo lote vencido custa 2 moedas a menos.", type: "discount", once: "act", value: 2 } },
  ...
  { id: "fallen-king-armor", name: "Armadura do Rei Caído", epithet: "A couraça que permaneceu de pé diante do cadáver", icon: "♝", prestige: 3, start: 3, tags: ["Realeza", "Guerra"], cursed: true, curse: { name: "Ferrugem do Trono", description: "Vale −1 Prestígio sem outra relíquia de Realeza.", penalty: 1 }, lore: "O metal ainda se curva diante de uma coroa que não existe.", power: { name: "Última Guarda", description: "Anule o próximo poder direcionado contra seu Museu.", type: "shield", once: "act" } },
  { id: "last-decree-scepter", name: "Cetro do Último Decreto", epithet: "A última ordem de um reino já morto", icon: "⚜", prestige: 3, start: 3, tags: ["Realeza", "Fé"], lore: "O selo na ponta ainda exige obediência.", power: { name: "Tributo Real", description: "O próximo lote começa 2 moedas mais caro; se um rival vencer, você recebe 2 moedas.", type: "royalDecree", once: "act" } },
...
function executeRelicAction(game: GameState, actorId: string, relicId: string, targetId?: string): GameState {
  ...
  if (!actionAvailable(relic) || !actionTimingAvailable(relic, game.status) || actor.artifactsUsedAct >= 2 + actor.extraArtifactsAct || !targetReady) return game;
  ...
  if (relic.power.type === "tax") {
    ...
    actor.prestigeBonus += amount; message = `O Espelho concedeu ${amount} Prestígio a ${actor.character.name}.`;
    ...
    if (success && target.inventory.length > 0) {
      ...
      message = `O Cavaleiro Oco roubou ${stolen.name} de ${target.character.name} e retornou ao Museu.`;
    } else {
    ...
    message = `${actor.character.name} presenteou ${target.character.name} com a Maçã Envenenada.`;
  } else if (relic.power.type === "exalt") {
    const amount = Math.max(2, Math.floor(actor.inventory.length / 2)); actor.prestigeBonus += amount;
    message = `O Trono elevou ${actor.character.name} em ${amount} Prestígio.`;
    ...
    message = `O Cetro aumentará em 2 moedas o início do próximo lote.`;
  } else if (relic.power.type === "tradeMark" && target) {
    target.tradeTributeTo = actor.id;
    message = `${target.character.name} foi marcado pela Duquesa e pagará tributo na próxima negociação.`;
    ...
    if (Math.random() < .5) { actor.gold += 5 + goldenBonus; message = `A Moeda sorriu: ${actor.character.name} recebeu ${5 + goldenBonus} moedas.`; }
    else { const lost = Math.min(3, actor.gold); actor.gold -= lost; message = `A Moeda traiu ${actor.character.name}, que perdeu ${lost} moedas.`; }
  } else if (relic.power.type === "judgment" && target) {
    if (target.gold >= 4) { const stolen = Math.min(2, target.gold); target.gold -= stolen; actor.gold += stolen; message = `${target.character.name} pagou ${stolen} moedas ao Carrasco.`; }
    else { target.prestigeBonus -= 1; actor.prestigeBonus += 1; message = `O Carrasco tomou 1 Prestígio de ${target.character.name}.`; }
    ...
    message = `O Estandarte aumentou em ${relic.power.value ?? 25}% o próximo ritual arriscado.`;
    ...
    } else if (relic.id === "fusion-crypt-judgment" && target) { if (target.gold >= 4) { target.gold -= 2; actor.gold += 2; message = `A Cripta tomou 2 moedas de ${target.character.name}.`; } else { target.prestigeBonus -= 1; actor.prestigeBonus += 1; message = `A Cripta tomou 1 Prestígio de ${target.character.name}.`; }
    } else if (relic.id === "fusion-seven-finger-hand" && target) {
      actor.riskBonus = 0;
      if (Math.random() <= Math.min(.95, .75 + markedChance)) { const stolen = Math.min(3, target.gold); target.gold -= stolen; actor.gold += stolen; relic.awakenings = (relic.awakenings ?? 0) + 1; if (relic.awakenings === 2) relic.bonusPrestige = (relic.bonusPrestige ?? 0) + 2; message = `A Mão roubou ${stolen} moedas${relic.awakenings === 2 ? " e despertou +2 Prestígios" : ""}.`; } else message = `A Mão dos Sete Dedos falhou.`;
    ...
    } else if (relic.id === "fusion-red-moon-baptism" && ownTargetRelic) { ownTargetRelic.curseSuppressed = true; ownTargetRelic.usedAct = false; actor.extraArtifactsAct += 1; message = `${ownTargetRelic.name} foi purificada, recarregada e liberada como terceira ativação.`;
...
export default function Home() {
  ...
  if (game.phase === "intro") return <Intro profile={profile} onEnter={() => setGame((current) => ({ ...current, phase: !profile.characterId ? "select" : profile.unlockedTalents.length === 0 ? "talents" : "lobby" }))} onRules={() => setRulesOpen(true)} onLogout={logout} rulesOpen={rulesOpen} closeRules={() => setRulesOpen(false)} />;
...
function Intro({ profile, onEnter, onRules, onLogout, rulesOpen, closeRules }: { profile: Profile; onEnter: () => void; onRules: () => void; onLogout: () => void; rulesOpen: boolean; closeRules: () => void; }) {
  const character = CHARACTERS.find((item) => item.id === profile.characterId);
  return <main className="opening-screen"><div className="ambient-glow" /><button className="account-logout" onClick={onLogout}>Sair de {profile.username}</button><section className="opening-card"><p className="eyebrow">O Baile das Máscaras Online apresenta</p><h1>Leilão da<br />Meia-Noite</h1><div className="gold-rule"><span>✦</span></div><p className="opening-copy">Crie uma sala para três ou quatro amigos, dispute relíquias proibidas e acompanhe cada lance ao vivo.</p>{character ? <div className="bound-character"><span>{character.sigil}</span><div><small>Sua máscara permanente</small><strong>{character.name}</strong><p>{character.title}</p></div></div> : <div className="character-preview">{CHARACTERS.map((item) => <article className="portrait-card" key={item.id}><div className="mask-mark">{item.sigil}</div><h2>{item.name}</h2><span>{item.title}</span></article>)}</div>}<div className="opening-actions"><button className="primary-button" onClick={onEnter}>{character && profile.unlockedTalents.length > 0 ? "Ver salas online" : character ? "Escolher primeiro talento" : "Escolher minha máscara"}</button><button className="text-button" onClick={onRules}>Como jogar</button></div><p className="mode-note">✦ {profile.lumens} Lúmens · {profile.wins} vitórias · {profile.unlockedTalents.length} talentos</p></section>{rulesOpen && <RulesModal onClose={closeRules} />}</main>;
}
...
function RelicDetailModal({ relic, player, status, enabled, onClose, onUse }: { relic: OwnedRelic; player: Player; status: GameStatus; enabled: boolean; onClose: () => void; onUse: () => void; }) {
  const limit = 2 + player.extraArtifactsAct;
