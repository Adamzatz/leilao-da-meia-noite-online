...
export const LEGENDARY_RELICS: Relic[] = [
  ...
  { id: "eclipsed-moon-mask", name: "Máscara da Lua Devorada", epithet: "O rosto usado pela noite durante o eclipse", icon: "◐", prestige: 5, start: 4, tags: ["Oculto", "Desejo"], legendary: true, lore: "Quando ela sorri, uma cadeira do salão amanhece vazia.", power: { name: "Banimento do Eclipse", description: "Um rival fica fora do próximo leilão.", type: "silence", once: "game", target: true } },
  { id: "first-phoenix-ashes", name: "Cinzas da Primeira Fênix", epithet: "O resto ardente do primeiro renascimento", icon: "♨", prestige: 5, start: 4, tags: ["Fé", "Morte"], legendary: true, lore: "As cinzas recusam o vento e procuram um novo dono.", power: { name: "Renascimento Imaculado", description: "Anule a maldição da próxima relíquia amaldiçoada.", type: "ward", once: "game" } },
  { id: "crowned-leviathan-pearl", name: "Pérola do Leviatã Coroado", epithet: "Tributo arrancado do fundo do oceano", icon: "◉", prestige: 6, start: 5, tags: ["Riqueza", "Realeza"], legendary: true, lore: "Toda fortuna do salão parece pequena diante de seu brilho.", power: { name: "Muralha das Profundezas", description: "Anule o próximo poder direcionado contra seu Museu.", type: "shield", once: "game" } },
  { id: "book-final-names", name: "Livro dos Últimos Nomes", epithet: "O registro encerra destinos antes de escrevê-los", icon: "▥", prestige: 5, start: 4, tags: ["Oculto", "Traição"], legendary: true, lore: "A última página sempre traz o nome de alguém presente.", power: { name: "Riscar da História", description: "Roube 2 Prestígios de um rival.", type: "siphon", once: "game", target: true, value: 2 } },
];
...
export const FUSION_RECIPES: FusionRecipe[] = [
  ...
  { id: "recipe-red-moon-baptism", components: ["moon-blood-vial", "broken-chain-rosary"], tier: 2, cost: 2, result: makeFusion("fusion-red-moon-baptism", "Batismo da Lua Rubra", "◒", 6, ["Oculto", "Desejo", "Fé"], "Renascimento Profano", "Purifique e recarregue um artefato; você recebe uma ativação extra neste ato.", "game", "ownRelic") },
...
function RelicLibraryCard({ relic, forbidden = false }: { relic: Relic; forbidden?: boolean }) {
  return <article className={`library-card relic-library-card ${relic.cursed ? "cursed" : ""} ${forbidden ? "forbidden" : ""}`}><header><span className="library-icon">{relic.icon}</span><div><small>{forbidden ? "ITEM PROIBIDO · candidato à votação" : relic.epithet}</small><h2>{relic.name}</h2><p>{forbidden ? relic.epithet : `“${relic.lore}”`}</p></div><b>✦ {relic.prestige}</b></header><div className="library-statline"><span>Lance inicial <b>{relic.start} ●</b></span><span>Uso <b>1× por {relic.power.once === "act" ? "ato" : "partida"}</b></span></div><div className="library-power"><small>{relic.power.name}</small><p>{relic.power.description}</p></div>{relic.curse && <div className="library-curse">☠ <strong>{relic.curse.name}</strong> · {relic.curse.description}</div>}<footer>{relic.tags.map((tag) => <span key={tag}>{tag}</span>)}{relic.cursed && <span className="danger">Amaldiçoada</span>}</footer></article>;
}
